...
function shiftDays(date, numberOfDays) {
    ...
    shiftedDate.setDate(
        shiftedDate.getDate() + numberOfDays
    );
...
function createSeedUsers(referenceDate) {
    const createdAt = shiftDays(
        referenceDate,
        -45
    ).toISOString();

    const updatedAt = shiftDays(
        referenceDate,
        -2
    ).toISOString();
...
function createSeedExams(referenceDate) {
    ...
    const activeStart = setTime(
        shiftDays(referenceDate, -2),
        8
    );

    const activeEnd = setTime(
        shiftDays(referenceDate, 5),
        23,
        59
    );

    const closedStart = setTime(
        shiftDays(referenceDate, -25),
        8
    );

    const closedEnd = setTime(
        shiftDays(referenceDate, -20),
        23,
        59
    );

    const upcomingStart = setTime(
        shiftDays(referenceDate, 2),
        8
    );

    const upcomingEnd = setTime(
        shiftDays(referenceDate, 8),
        23,
        59
    );
    ...
    return [
        ...
        },
        {
            ...
            questions: [
                ...
                },
                {
                    ...
                    type: "true-false",
                    text:
                        "addEventListener can register a click handler on a button.",
                    code: "",
                    points: 5,
                    options: [
                        {
        ...
        },
        {
            ...
            startAt: setTime(
                shiftDays(referenceDate, 12),
                8
            ).toISOString(),
            endAt: setTime(
                shiftDays(referenceDate, 18),
                23,
                59
            ).toISOString(),
            durationMinutes: 45,
...
function createSeedAttempts(referenceDate) {
    const firstStartedAt = setTime(
        shiftDays(referenceDate, -23),
        10
    );

    const firstSubmittedAt = new Date(
        firstStartedAt.getTime() +
        18 * 60 * 1000
    );

    const secondStartedAt = setTime(
        shiftDays(referenceDate, -22),
        11
    );

    const secondSubmittedAt = new Date(
        secondStartedAt.getTime() +
        20 * 60 * 1000
    );
...
export function seedInitialData(
    ...
    if (attemptsWritten) {
        writeStorage(
            STORAGE_KEYS.ATTEMPTS,
            createSeedAttempts(referenceDate)
        );
    }
