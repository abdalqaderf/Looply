...
function getProfileElements() {
  ...
  if (
    ...
    throw new Error("Student profile elements are missing from the page.");
  }
...
function renderStudentProfile(elements, user) {
  ...
  if (!fullName || !username) {
    throw new Error("The current student profile is incomplete.");
  }
...
function renderProfileError(elements) {
  ...
  elements.avatar.setAttribute("aria-label", "Student profile unavailable");
}
...
function initializeStudentProfile() {
  ...
  try {
    ...
    renderStudentProfile(elements, user);
  } catch (error) {
    console.error("Unable to load the student profile.", error);
