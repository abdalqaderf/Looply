...
function getDashboardElements() {
    return {
        welcome:
            getElement("#welcome-message"),

        availableCount:
            getElement("#stat-available-count"),

        completedCount:
            getElement("#stat-completed-count"),

        availableList:
            getElement("#available-exams-container"),

        latestResult:
            getElement("#latest-result-container"),

        latestEmpty:
            getElement("#latest-result-empty-container"),

        latestEmptyMessage:
            getElement("#latest-result-empty-message"),

        latestIcon:
            getElement("#latest-result-icon-container"),

        latestTitle:
            getElement("#latest-result-title"),

        latestDate:
            getElement("#latest-result-date"),

        latestScore:
            getElement("#latest-result-score"),

        latestMax:
            getElement("#latest-result-max"),

        latestFraction:
            getElement("#latest-result-fraction"),

        latestStatus:
            getElement("#latest-result-status"),

        latestLink:
            getElement("#latest-result-review-link")
    };
}
...
function getAvailableExams(attempts) {
    const submittedExamIds = new Set(
        getSubmittedAttempts(attempts).map(
    ...
    return getActiveExams()
        ...
        .sort((first, second) => (
            new Date(first.endAt).getTime() -
            new Date(second.endAt).getTime()
        ));
}
...
function renderLoading(elements) {
    ...
    hideElement(
        elements.latestResult
    );

    showElement(
        elements.latestEmpty
    );

    setText(
        elements.latestEmptyMessage,
...
function renderLatestResult(
    ...
    setText(
        elements.latestScore,
        safeNumber(attempt.score)
    );

    setText(
        elements.latestMax,
        safeNumber(attempt.totalScore)
    );
    ...
    setText(
        elements.latestStatus,
        passed
            ? "Passed"
            : "Failed"
    );

    elements.latestStatus.classList.toggle(
        "student-result-passed",
        passed
    );

    elements.latestStatus.style.color =
        passed
            ? ""
            : "var(--danger)";
    ...
    elements.latestIcon.style.color =
        passed
            ? ""
            : "var(--danger)";
    ...
    elements.latestLink.setAttribute(
        "aria-label",
...
function initializeStudentDashboard() {
    ...
    try {
        ...
        try {
            ...
            console.error(
                "Unable to display the dashboard error alert.",
                alertError
            );
        }
    }
