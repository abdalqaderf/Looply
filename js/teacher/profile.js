...
import { getElements, getOptionalElement, setText } from "../core/dom.js";
...
function getProfileElements() {
  ...
  if (
    ...
    throw new Error("Teacher profile elements are missing from the page.");
  }
...
function renderTeacherProfile(elements, user) {
  const fullName = normalizeText(user.fullName);
  const username = normalizeText(user.username);

  if (!fullName || !username) {
    throw new Error("The current teacher profile is incomplete.");
  }

  const initials = getInitials(fullName) || "TR";

  setAllText(elements.names, fullName);
  setAllText(elements.usernames, username);
  setAllText(elements.roles, "Teacher");
  setText(elements.initials, initials);

  elements.avatar.setAttribute("aria-label", `${fullName} initials`);
}

function renderProfileError(elements) {
  setAllText(elements.names, "Profile unavailable");

  setAllText(elements.usernames, "Unable to load username");

  setAllText(elements.roles, "Teacher");

  setText(elements.initials, "--");

  elements.avatar.setAttribute("aria-label", "Teacher profile unavailable");
}

function initializeTeacherProfile() {
  const user = requireRole(USER_ROLES.TEACHER, {
    redirect: true,
  });
  ...
  try {
    ...
    renderTeacherProfile(elements, user);
  } catch (error) {
    console.error("Unable to load the teacher profile.", error);
    ...
    try {
      showErrorToast("Unable to load your profile.");
    } catch (alertError) {
      console.error("Unable to display the profile error alert.", alertError);
    }
  }
}
