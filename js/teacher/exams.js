import { APP_CONFIG, EXAM_STATUS, ROUTES, USER_ROLES } from "../core/config.js";
...
function actionLink(route, exam, label, iconName) {
  return make(
    "a",
    {
      ...
      attributes: {
        href: routeWithExamId(route, exam.id),

        title: `${label} exam`,

        "aria-label": `${label} ${exam.title || "exam"}`,
      },
    },
...
function deleteButton(exam) {
  return make(
    "button",
    {
      className: "exam-action-btn exam-delete-btn",
      ...
      attributes: {
        ...
        "aria-label": `Delete ${exam.title || "exam"}`,
      },
  ...
  return make(
    "tr",
    {
      dataset: {
        ...
        examTitle: normalizeText(exam.title).toLowerCase(),
    ...
    make(
      ...
      {},
      make("span", {
        className: `exam-status exam-status-${status}`,
    ...
    make(
      ...
      make(
        ...
        actionLink(ROUTES.TEACHER_EXAM_DETAILS, exam, "View", "bi-eye"),

        actionLink(ROUTES.TEACHER_EXAM_FORM, exam, "Edit", "bi-pencil"),

        statusButton(exam, status),
...
function visibleExams(searchValue) {
  ...
  return exams.filter((exam) => {
    ...
    return [exam.title, exam.description, exam.instructions, status]
...
function render(elements) {
  const visible = visibleExams(elements.search.value);
  ...
  if (visible.length === 0) {
    const filtering =
      activeFilter !== "all" || normalizeText(elements.search.value);
  ...
  visible.forEach((exam) => {
    elements.body.append(examRow(exam));
  });
}

function updateFilters(elements) {
  elements.filters.forEach((button) => {
    const selected = button.dataset.examFilter === activeFilter;
...
function reload(elements, teacherId) {
  exams = getExamsByTeacher(teacherId);

  render(elements);
}

async function toggleStatus(elements, teacherId, button) {
  const exam = exams.find((item) => item.id === button.dataset.examId);

  const nextStatus = button.dataset.nextStatus;

  if (!exam) {
    showErrorToast("Exam was not found.");
  ...
  try {
    changeExamStatus(exam.id, nextStatus);

    reload(elements, teacherId);
...
async function removeExam(elements, teacherId, button) {
  const exam = exams.find((item) => item.id === button.dataset.examId);
  ...
  const confirmed = await confirmDelete(exam.title || "this exam");
...
function bindEvents(elements, teacherId) {
  elements.search.addEventListener("input", () => render(elements));

  elements.filters.forEach((button) => {
    button.addEventListener("click", () => {
      activeFilter = button.dataset.examFilter || "all";
  ...
  elements.body.addEventListener("click", (event) => {
    ...
    if (button.dataset.examAction === "toggle") {
      void toggleStatus(elements, teacherId, button);
    } else if (button.dataset.examAction === "delete") {
      void removeExam(elements, teacherId, button);
    }
  });
}
...
function initializeTeacherExams() {
  ...
  try {
    ...
    elements.body.append(stateRow("Loading exams..."));

    updateFilters(elements);

    bindEvents(elements, teacher.id);

    reload(elements, teacher.id);
  } catch (error) {
    console.error("Unable to initialize the exams page.", error);
    ...
    if (elements) {
      ...
      elements.body.append(stateRow("Unable to load exams."));
    }

    showErrorToast("Unable to load the exams page.");
  }
}

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeTeacherExams, {
    once: true,
  });
} else {
