...
export function getExamById(
    ...
    return getAllExams().find((exam) => (
        exam.id === normalizedId &&
...
function normalizeExamData(data = {}, currentExam = null) {
    ...
    const questions = data.questions === undefined
        ? currentExam?.questions ?? []
        : normalizeQuestions(data.questions);

    return {
        teacherId: normalizeText(
            data.teacherId ?? currentExam?.teacherId
        ),
        title: normalizeText(data.title ?? currentExam?.title),
        description: normalizeText(
            data.description ?? currentExam?.description
        ),
        instructions: normalizeText(
            data.instructions ?? currentExam?.instructions
        ),
