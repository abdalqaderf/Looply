import { ATTEMPT_STATUS, EXAM_STATUS, STORAGE_KEYS } from "../core/config.js";
...
import { getExamById, getExamTotalPoints } from "./exams-service.js";
...
export function getAttemptById(attemptId) {
  ...
  return (
    getAllAttempts().find((attempt) => attempt.id === normalizedId) ?? null
  );
}
...
export function getStudentExamAttempt(studentId, examId) {
  return (
    getAttemptsByStudent(studentId).find(
...
export function hasStudentSubmittedExam(studentId, examId) {
  return getAttemptsByStudent(studentId).some(
    (attempt) =>
      attempt.examId === examId && attempt.status === ATTEMPT_STATUS.SUBMITTED,
  );
}
...
function normalizeComparableAnswer(answer) {
  ...
  return normalizeText(answer).replace(/\r\n/g, "\n").toLowerCase();
}
...
export function calculateScore(exam, answers = {}) {
  ...
  exam.questions.forEach((question) => {
    const studentAnswer = normalizeComparableAnswer(answers[question.id]);

    const correctAnswer = normalizeComparableAnswer(question.correctAnswer);
...
export function saveAnswer(attemptId, questionId, answer) {
  ...
  const answerIsEmpty =
    answer === null || answer === undefined || normalizeText(answer) === "";
